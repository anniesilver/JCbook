/**
 * TEST G: FULLY AUTOMATED WITH FRESH TOKEN
 *
 * This test:
 * 1. Automates login (no manual intervention)
 * 2. Loads booking form
 * 3. Calls grecaptcha.execute() to get FRESH token
 * 4. Closes browser
 * 5. Submits via HTTP POST with fresh token
 *
 * Goal: Test if fresh token from grecaptcha.execute() works with HTTP POST
 */
...
async function runTest() {
  console.log('\n' + '█'.repeat(80));
  console.log('TEST G: FULLY AUTOMATED WITH FRESH TOKEN');
  ...
  try {
    ...
    // ===================================================================
    console.log('\n' + '═'.repeat(80));
    console.log('TEST G RESULT - ANALYSIS');
    console.log('═'.repeat(80));
